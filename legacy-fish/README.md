...
- Recent Claude Code releases keep a background **daemon** that caches each
  ...
  restored credential, while any detached background sessions keep running. It
  is best-effort: on an older Claude Code, or when no daemon is running, it is a
  harmless no-op and the switch proceeds regardless.
